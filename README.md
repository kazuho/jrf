...
- `-P N` opportunistically parallelizes map stages across `N` worker processes when multiple input files are provided. The pipeline is automatically split at the first reducer stage: map stages run in parallel workers, and reducer stages run in the parent process. If parallelization does not apply cleanly, execution falls back to single-threaded. Processing order is not guaranteed under `-P`, so order-sensitive reducers may produce different results than serial execution. Errors in individual workers (e.g., truncated `.gz` files) are reported to stderr without aborting the entire run.
