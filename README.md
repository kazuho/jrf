...
[^note]: In `jq`, calculating the minimum over multiple objects can be written as `jq -s 'map(.tid) | min` too, however that does not scale as it buffers the entire input.
